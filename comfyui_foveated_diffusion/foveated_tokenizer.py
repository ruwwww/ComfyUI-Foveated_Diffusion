...
def build_foveated_tokens(
    ...
    # Slice the top-left token (index 0) of each block directly
    img_down = img_blocks[:, :, :, 0, :]

    import logging
    logger = logging.getLogger("comfyui_foveated_diffusion")
    logger.info(
        "FoveatedTokenizer Debug: (Top-Left Subsampling) img shape=%s, img_down shape=%s, lr_factor=%d",
